...
static NValue
toggle_bool(void* data, NValue*, uint8_t, NError* error);

static NValue
wrap_input(void* data, NValue*, uint8_t, NError* error);
...
TEST(execution_of_primitive_sees_data) {
	...
	n_primitive_call(primitive, NULL, 0, &error);
...
TEST(execution_of_primitive_returns_function_result) {
	...
	NValue result = n_primitive_call(primitive, NULL, 0, NULL);
...
static NValue
toggle_bool(void* data, NValue* args, uint8_t nargs, NError* error) {
	NValue result;
